...
import { setDeleteBook, setToggleFavorite, selectBooks } from "../../redux/slices/booksSlice";
import { selectTitleFilter, selectOnlyFavoriteFilter, selectAuthorFilter } from "../../redux/slices/filterSlice";
import "./BookList.css";
         ...
         {filteredBooks.length === 0 ? (<p>No books available</p>) : (
            <ul>
               {filteredBooks.map((book, i)=>(
                  <li key={book.id}>
                     <div className="book-info">{++i}. {highlightMatch(book.title, titleFilter)} by <strong>{highlightMatch(book.author, authorFilter)}</strong> {book.year} ({book.source})</div>
